...
class StandingsPresenter: StandingsPresenterProtocol {
    ...
    func loadData(leagueId: String, seasonYear: String) {
        NetworkService.shared.dataRequest(with: "\(Constant.mainUrl)/leagues/\(leagueId)/standings?season=\(seasonYear)&sort=asc", objectType: LeagueStandings.self) { [weak self] result in
            switch result {
            case .success(let object):
                self?.contentView.standings = object.data.standings
